...
export function useSession(requiredRole?: Role) {
  ...
  useEffect(() => {
    ...
    const isAdminish = user.role==='admin' || user.role==='superadmin'; if (requiredRole && user.role !== requiredRole && !(requiredRole==='admin' && isAdminish)) router.replace(user.role==='educator'?'/educator/classes':isAdminish?'/admin/overview':'/participant/home');
