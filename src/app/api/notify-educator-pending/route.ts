...
export async function POST(req: NextRequest) {
  try {
    ...
    const fromName = process.env.BREVO_FROM_NAME || 'QuestMaster';
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://indoorgame.airiz.tech';
    const approveUrl = `${siteUrl}/admin/users`;

    const subject = `[QuestMaster] New educator pending approval: ${name || email}`;
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f9fafb;">
        <div style="background:#fff;border-radius:12px;padding:24px;border:1px solid #e5e7eb;">
          ...
          <p style="color:#374151;">A new educator has registered on QuestMaster and is awaiting your approval.</p>
          ...
          <p style="color:#9ca3af;font-size:12px;margin-top:24px;">QuestMaster Admin Notification</p>
        </div>
      </div>`;
