...
export async function POST(req: NextRequest) {
  try {
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://cendekia.airiz.tech';
    const approveUrl = `${siteUrl}/admin/users`;

    const subject = `[Cendekia] New educator pending approval: ${name || email}`;
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f9fafb;">
        <div style="background:#fff;border-radius:12px;padding:24px;border:1px solid #e5e7eb;">
          ...
          <p style="color:#374151;">A new educator has registered on Cendekia and is awaiting your approval.</p>
          ...
          <p style="color:#9ca3af;font-size:12px;margin-top:24px;">Cendekia Admin Notification</p>
        </div>
      </div>`;
